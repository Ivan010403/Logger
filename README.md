...
# Краткая информация
Проект представляет собой исходный код библиотеки и тестирующего приложения. Для компиляции используется компилятор GCC с флагами компиляции `-std=c++17 -Wall -Wextra`. Автор целенаправленно не использовал другие возможности компилятора (к примеру всякого рода оптимизации).

Важный момент: при сборке проекта и создании динамической библиотеки путь для динамического линковщика указывается через `-Wl,-rpath,`, т.е. помогать системе с поиском динамической библиотеки (посредством переменной `LD_LIBRARY_PATH`) не нужно.
...
В классе определены два конструктора: конструктор с параметрами `Logger(const std::string, LoggerLevel)` и конструктор копирования `Logger(const Logger&)`. Причина, по которой было принято решение реализовать конструктор копирования, связана с передачей логера внутрь функций ("прокидывание" логера). **Мы крайне советуем использовать передачу по ссылке** (или даже константной ссылке), дабы избежать лишнего копирования, но всё-таки оставляем возможность передавать объект класса по значению и создавать копии. 
...
Деструктор объявлен как деструктор по умолчанию и не выполняет никаких дополнительных действий `~Logger() = default`.
